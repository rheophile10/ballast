...
const arrivals = {
  rtcRegistrations: async () => (await cast.registrationsOf(await Promise.all([0, 1, 2].map(cast.rtc)))).map((f) => ['Inbox', f]),
...
const arrive = async (key) => {
  ...
  let s = st; for (const [folder, f] of items) s = os.addFile(s, folder, f.name, f.text, f.from);
...
const act = async (kind, ...a) => {
  ...
  if (kind === 'open-file') {
    ...
    setTimeout(() => tell({ type: 'open', text: f.text, name: f.name }), 150);
    if (/^registration-/.test(f.name)) set({ ...st, pendingSeen: st.pendingSeen + 1 });
    if (/^class-profile-/.test(f.name)) set({ ...st, reportsSeen: st.reportsSeen + 1 });
    if (/^release-/.test(f.name)) set({ ...st, releasesSeen: st.releasesSeen + 1, appealSeen: st.appealSeen || f.folder === 'Inbox' });
  }
};
