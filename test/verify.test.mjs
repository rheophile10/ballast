...
import { copyTest, issueTest } from '../src/testfile.js';
...
test('verify.py agrees with src/ on a real release', async () => {
  ...
  const { text: tgbo, record } = await issueTest(t, sheet.rtc, sheet.trains, { 'clearance-134.png': { mime: 'image/png', b64: 'AA==' } });
  sheet.tests.push(record);
  const copy = await copyTest(tgbo, train, train.pin);
