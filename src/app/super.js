...
const authorBox = () => { const ta = h('textarea', { rows: 14, value: editor.text, placeholder: '# Title\ntime: 60m\npass: 90%\nareas: signals "Signals" · switches "Switches"\n\n---\ntype: mc\narea: signals\nref: CROR 27(b)\nQ: …\n*a) right\nb) wrong\n', style: { fontFamily: 'monospace', fontSize: '13px', maxWidth: 'none' }, oninput: (e) => { editor.text = e.target.value; } });
  const add = (tpl) => { editor = { ...editor, text: (editor.text.trim() ? editor.text.replace(/\s+$/, '') + '\n---\n' : '# New test\ntime: 30m\npass: 90%\nshuffle: items options\nback: no\n\n---\n') + tpl }; rerender(); };
  const T = { mc: 'type: mc\nref: CROR \nQ: \n*a) \nb) \nc) \n', match: 'type: match\nref: CROR \nQ: Match each term with its meaning.\nL:  = \nL:  = \nL:  = \n', short: 'type: short\nref: CROR \nworth: 2\nQ: \nA: \n' };
  return h('div', { class: 'card' }, h('h2', {}, 'Write a test'), h('p', { class: 'small' }, 'The source format is in SPEC.md §10: a title and settings, then items separated by --- lines. It is plain text — Notepad works as well as this box. Validate shows problems; Add puts it in the inventory, where you can approve it.'),
    h('div', { class: 'row' }, h('button', { onclick: () => add(T.mc) }, '+ multiple choice'), h('button', { onclick: () => add(T.match) }, '+ matching'), h('button', { onclick: () => add(T.short) }, '+ short answer')), ta,
