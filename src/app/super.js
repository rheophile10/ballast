...
const inventoryTab = () => h('div', {}, h('p', { class: 'small' }, 'Drop test sources. Approve signs the source\'s hash; give the approval file to the RTC, who drops it on their desk. A test built from a byte-identical source then carries the approval.'),
...
export const render = async (r, ctx, g) => {
  ...
  const drop = h('div', { class: 'drop', ondragover: (e) => { e.preventDefault(); drop.classList.add('over'); }, ondragleave: () => drop.classList.remove('over'), ondrop: (e) => { e.preventDefault(); drop.classList.remove('over'); takeFiles(e.dataTransfer.files); } }, 'Drop RTC registrations, test sources, reports here', h('br'), h('label', { class: 'btn', style: { marginTop: '8px', display: 'inline-block' } }, 'Choose files', h('input', { type: 'file', multiple: true, style: { display: 'none' }, onchange: (e) => takeFiles(e.target.files) })));
