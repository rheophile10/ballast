...
export const sendReport = async (teacher, directorPub, report) => {
  const salt = unb64url(report.tgbo); // the TGBO id doubles as the HKDF salt for its report
  const key = await hkdfKey(await shared(teacher.priv, directorPub), salt, INFO.report);
  return armor('REPORT', { v: 1, kind: 'report', tgbo: report.tgbo, teacher: teacher.pub, box: await sealJSON(key, report) }, { title: report.title, teacher: teacher.name, trains: report.rows.length });
};
export const takeReport = async (text, director) => {
  const { body } = await dearmor(text, 'REPORT');
  const key = await hkdfKey(await shared(director.priv, body.teacher), unb64url(body.tgbo), INFO.report);
